...
def mlp(sizes, act_fun, output_activation=nn.Identity):
    layers = []
    for j in range(len(sizes) - 1):
        act = act_fun if j < len(sizes) - 2 else output_activation
...
class MLPActor(nn.Module):
    ...
    def _log_prob_of_act_from_distribution(self):
        logp_a = self.pi.log_prob(self.act).sum(axis=-1)
        return logp_a
...
class PBOBuffer:
    ...
    def finish_path(self, last_val=0):
        ...
        self.rew_buf[path_slice] = core.discount_cumsum(rews, self.gamma)[:-1]
